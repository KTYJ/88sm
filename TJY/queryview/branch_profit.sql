...
cl scr
ACCEPT no_years NUMBER PROMPT 'Enter number of years for profitability report [2]: ' DEFAULT 2
...
WHERE o.order_date >= ADD_MONTHS(SYSDATE, -(&no_years * 12))
...
FROM procurement
WHERE procurement_date >= ADD_MONTHS(SYSDATE, -(&no_years * 12))
GROUP BY item_id;
...
SELECT
    TO_CHAR(ADD_MONTHS(SYSDATE, -(&no_years * 12)), 'YYYY') AS start_year,
