...
CREATE OR REPLACE PROCEDURE proc_place_order (
    ...
    d_address    IN  delivery.delivery_address%TYPE DEFAULT NULL -- optional if delivery
) IS
...
BEGIN
    ...
    IF p_order_type = 'Delivery' THEN
        IF d_address IS NULL THEN
            SELECT address INTO v_final_address FROM member WHERE member_id = p_member_id;
            IF v_final_address IS NULL THEN
                RAISE e_missing_address;
            END IF;
        ELSE
            v_final_address := d_address;
        END IF;
...
EXCEPTION
    ...
    WHEN e_missing_address THEN
        ROLLBACK;
        RAISE_APPLICATION_ERROR(-20010, 'Delivery address is required but none was provided and member has no default address.');
